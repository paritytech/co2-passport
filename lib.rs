...
mod asset_co2_emissions {
    ...
    impl AssetCO2Emissions for InfinityAsset {
        ...
        fn blast(
            ...
            self.save_new_emission(&asset_id, &emissions)?;
        ...
        fn transfer(
            ...
            // Save CO2 emissions & emit corresponding events
            self.save_new_emission(&id, &emissions)?;
        ...
        fn add_emissions(
            ...
            // Save CO2 emissions & emit corresponding events
            self.save_new_emission(&id, &Vec::from([emissions]))?;
            Ok(())
        }
    ...
    mod tests {
        ...
        fn new_emissions(start: u8, end: u8) -> Vec<CO2Emissions> {
        ...
        fn get_default_emission() -> CO2Emissions {
        ...
        fn blast_default_asset(contract: &mut InfinityAsset) {
            let accounts = get_accounts();
            let metadata: Metadata = Vec::from([0u8, 1u8, 2u8, 3u8]);
            let parent = None;

            let item = get_default_emission();
        ...
        fn should_reject_zero_emissions_item_in_array_during_blast() {
            ...
            let emissions = new_emissions(0, 3);
        ...
        fn should_blast_with_multiple_emissions_items() {
            ...
            let start_id = 1;
            let items_count = 3u8;
            let emissions = new_emissions(start_id, items_count + 1);
            ...
            assert_eq!(1 + items_count, test::recorded_events().count() as u8);
            assert_blasted_event(
                &emitted_events[0],
                expected_asset_id,
                metadata,
        ...
        fn should_get_emissions_work_properly() {
            ...
            let start_id = 1;
            let end_id = 3;
            let emissions = new_emissions(start_id, end_id);
        ...
        fn should_get_metadata_work_properly() {
            ...
            blast_default_asset(&mut contract);
        ...
        fn should_reject_0_parent_relation_in_blast() {
            ...
            let metadata = Vec::from([0u8, 1u8, 2u8, 3u8]);
            let emissions = Vec::from([get_default_emission()]);
        ...
        fn should_not_owner_not_be_able_to_add_emissions() {
            ...
            let new_emission_item = get_default_emission();
        ...
        fn should_reject_paused_in_add_emissions() {
            ...
            assert_eq!(
                contract.add_emissions(asset_id, e),
                Err(AssetCO2EmissionsError::AlreadyPaused)
            );
        }
        ...
        fn should_reject_zero_emissions_item_in_add_emissions() {
            ...
            let emissions_value_0 = 0u128;
            let mut new_emission_item = get_default_emission();
            new_emission_item.value = emissions_value_0;
        ...
        fn should_owner_be_able_to_add_emissions() {
            ...
            let mut new_emission_item = get_default_emission();
            new_emission_item.value = emissions_value_1;

            assert!(contract
                .add_emissions(asset_id, new_emission_item.clone())
                .is_ok());

            let emitted_events = test::recorded_events().collect::<Vec<_>>();

            let e = new_emission_item.clone();
            ...
            let expected_emissions: Vec<CO2Emissions> = Vec::from([e, new_emission_item]);
            let emissions_from_state = contract.get_asset_emissions(asset_id);
            assert!(emissions_from_state.is_some());
        ...
        fn should_not_owner_not_be_able_to_transfer() {
            ...
            assert_eq!(
                contract.transfer(accounts.bob, asset_id, Vec::from([new_emission_item])),
                Err(AssetCO2EmissionsError::NotOwner)
            );
        }
        ...
        fn should_reject_too_many_emissions_on_blast() {
            ...
            for _ in 0..MAX_EMISSIONS_PER_ASSET + 1 {
                emissions.push(get_default_emission());
            }
        ...
        fn should_reject_zero_emissions_item_in_transfer() {
            ...
            let mut new_emission_item_0 = get_default_emission();
            new_emission_item_0.value = emissions_value_1;

            let new_emission_item_1 = get_default_emission();
            let new_emission_item_2 = get_default_emission();
        ...
        fn should_owner_be_able_to_transfer() {
            ...
            let mut new_emission_item = get_default_emission();
            new_emission_item.category = EmissionsCategory::Transport;
            new_emission_item.value = emissions_value_1;

            assert!(contract
                .transfer(new_owner, asset_id, Vec::from([new_emission_item.clone()]))
                .is_ok());
            ...
            let expected_emissions: Vec<CO2Emissions> = Vec::from([item, new_emission_item]);
            let emissions_from_state = contract.get_asset_emissions(asset_id);
            assert!(emissions_from_state.is_some());
            assert!(expected_emissions
                .iter()
        ...
        fn should_query_emissions_for_longer_path_work_properly() {
            ...
            let emissions: Vec<CO2Emissions> = Vec::from([get_default_emission()]);
        ...
        fn should_list_asset_after_transfer_work_properly() {
            ...
            assert!(contract
                .transfer(new_owner, asset_id, Vec::from([new_emission_item]))
                .is_ok());
